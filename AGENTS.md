...
│  ├─ AdvisorAgent     （Codex-agent-sdk query 包装）        │
...
│   │   │   ├── profile-store.ts
│   │   │   ├── history-store.ts
│   │   │   └── advisor-agent.ts    # Codex-agent-sdk 包装
│   │   ├── agents/                 # 见 §5
│   │   │   ├── orchestrator/
│   │   │   ├── data-curator/
│   │   │   ├── team-composer/
...
## 5. Agent 编排（Codex 介入点）

> 本节单独立章，因为 Codex-agent-sdk 是这个产品的"大脑"，分工必须明确。**核心原则：Agent 之间的边界 = 输入输出契约 + 系统提示词 + 可用工具集 三件套**，不允许任何 Agent 越权访问外部世界（外部 IO 必须走 Skill Tool）。
...
- Prompt 单测：`tests/unit/agents/<name>.spec.ts` —— 给定典型输入，断言 SDK 的 mock 返回能被 Agent 正确解析（用 `Codex-agent-sdk` 的测试 hook 或自写 mock query）。
...
- `baseUrl`（可选，默认 `https://api.anthropic.com`；兼容 Anthropic Messages 协议的代理在此填即可：智谱 / 魔搭 / OpenRouter / 自建 Codex 兼容网关）
- `model`（默认 `Codex-sonnet-4-6`，文本输入框可手改；下拉只给 known-good 列表作快捷选择）
...
- `Options.pathToClaudeCodeExecutable` 显式指向 `node_modules/@anthropic-ai/Codex-agent-sdk/cli.js`（dev 环境）或 `process.resourcesPath/app.asar.unpacked/node_modules/.../cli.js`（打包后）。
- `Options.executable: 'node'`，runtime 用 Electron 内置 Node。
- 通过 `disallowedTools: ['Bash', 'Edit', 'Write', 'WebFetch', ...]` 关掉所有不需要的 Codex 原生 tools，只保留我们自己 `createSdkMcpServer` 注册的业务工具。
...
[Main: AdvisorAgent]
   ├─ 读取 ProfileStore
   ├─ Codex-agent-sdk query() with userKey
...
- **图标**：元素图标使用 HoYoLAB 官方公开素材，确保玩家能直接映射到游戏内语义；官方素材必须集中放在 `resources/official/`，记录原始 URL、文件摘要、用途与权利归属。其他应用图标优先自绘或采用 CC0/OFL 资源。

**强制约束**
- 产品明确需要时可以采用官方公开视觉素材，但不得伪装为项目原创，也不得暗示本项目受米哈游或 HoYoverse 官方背书。
- 所有官方素材、装饰 SVG、字体和生成位图都必须在 `resources/credits.md` 列出来源；可下载的官方素材同时在 manifest 中记录 SHA-256。
...
- **v0.3 Advisor**：Codex-agent-sdk 接入，单环境推荐。
