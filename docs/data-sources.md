...
| Fandom 文本                                | 已公开玩法或机制的文字补充、人工核对线索                 | Fandom 默认文本许可为 [CC BY-SA 3.0](https://www.fandom.com/licensing)，但采集时仍要核对具体 wiki 页面的许可。采用或改写文本必须保留页面、作者/社区归属与相同方式共享说明；讨论区、聊天、图片和站点 UI 不在默认文本许可范围内，不采集。 | 不自动轮询；版本更新后由人工按需核对。每次采用文本时记录页面修订时间，每季度复查链接与许可，发现变化即停止重新发布直到审核完成。                                            |
...
账号数据只描述玩家状态，不作为全局玩法规则来源；官方公告与已审核知识库描述玩法规则。Enka 只补充 UID 展示柜资料，不能替代完整账号资料、活动规则或敌人轮换。
...
- 公共发布 payload 不能只由 `development-cross-check` 来源支持；至少要有一个允许用于发布的非开发来源。
...
- 来源类型、可访问时的原始 URL、抓取时间和必要署名可审计；
- 人工修正或多来源裁决写入字段备注；
- 重复来源 ID、悬空引用和仅由开发交叉源支持的 payload 一律拒绝。
