...
- 公共发布 payload 的实际字段不能只由 `development-cross-check` 来源支持；至少一条 `fieldProvenance` 必须引用允许发布的非开发来源。只声明但未被任何字段引用的 official source 不计入支持来源。
...
- 人工修正或多来源裁决写入字段备注；
- 重复来源 ID、悬空引用，以及实际字段仅引用开发交叉源的 payload 一律拒绝；未使用的非开发来源不能绕过该检查。
