...
`gate:advisor-saved` 还会检查 compose、critique、rotation、explain 四阶段都有脱敏后的模型原文和正数 usage；同名阶段出现多次时，每一次都必须完成并满足这些条件。trace 汇总 usage 必须为正数，最终两队恰好八名不重复 owned 角色，且结果的 knowledge summary 与同一 trace 完全一致。整个 `recommend` 调用有固定总时限；超时会先按 correlation ID 取消当前推荐并立即返回 `ADVISOR_TIMEOUT`，不会等待 research 或其他下游自行响应 abort。Web Search 只在可信知识有缺口时运行，结果仍只写入 24 小时 `ephemeral-web` 缓存；门禁没有可信知识 writer。正式场景刷新失败时，只要 last-known-good 正式发布仍为 fresh / expiring 且 usable，门禁可以继续；只有没有可用的 production publication 时才返回 `SCENARIO_UNAVAILABLE`，绝不改用 development sample。

Advisor 失败仍以稳定 gate code 为主；如果同一份 trace 提供了失败证据，JSON 只可附带 `failedStage`、`traceFailureCode`、`sdkCode`、`httpStatus` 这些经过白名单校验的诊断字段。`ADVISOR_FELL_BACK` 不会被底层异常码替换，但会尽量指出真实失败阶段。失败 message、Provider body、stage raw output、prompt 和 Profile 一律不进入门禁输出。
...
- Advisor gate：除保存配置和 `PROVIDER_ERROR` 外，还可能返回 `ADVISOR_TIMEOUT`、`ACTIVE_PROFILE_UNAVAILABLE`、`ROSTER_INSUFFICIENT`、`SCENARIO_UNAVAILABLE`、`ADVISOR_NOT_PLANNED`、`ADVISOR_FELL_BACK`、trace / stage / usage 缺失类 code、`TEAM_SIZE_INVALID`、`CHARACTER_NOT_OWNED`、`KNOWLEDGE_SUMMARY_MISMATCH` 或 `ADVISOR_LATENCY_INVALID`。
...
Maintainers have three manual saved-state gates. `gate:provider-saved` checks only Messages reachability, `gate:agent-saved` checks one real tool-free SDK child-process turn, and `gate:advisor-saved` checks the complete production-scenario Abyss pipeline without fallback. The latter two consume saved-provider usage and are intentionally excluded from tests and builds. Their stdout is one privacy-safe JSON line; they never print keys, custom-header values, provider bodies, prompts, stage raw output, or a full profile. The advisor gate applies one fixed deadline to the complete `recommend` call, cancels that correlation ID on expiry, and reports `ADVISOR_TIMEOUT` immediately. Every occurrence of every required stage must be completed with raw output and positive usage. Failures may add only allowlisted `failedStage`, `traceFailureCode`, `sdkCode`, and `httpStatus` diagnostics; `ADVISOR_FELL_BACK` remains the main code. If refresh fails, the advisor gate may continue with a usable `last-known-good` production publication whose freshness remains `fresh` or `expiring`; it reports `SCENARIO_UNAVAILABLE` only when no usable production publication exists and never substitutes a development fixture.
