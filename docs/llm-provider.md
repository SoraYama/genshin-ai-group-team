...
`gate:advisor-saved` 还会检查 compose、critique、rotation、explain 四阶段都有脱敏后的模型原文和正数 usage，trace 汇总 usage 为正数，最终两队恰好八名不重复 owned 角色，且结果的 knowledge summary 与同一 trace 完全一致。Web Search 只在可信知识有缺口时运行，结果仍只写入 24 小时 `ephemeral-web` 缓存；门禁没有可信知识 writer。正式场景刷新失败时，只要 last-known-good 正式发布仍为 fresh / expiring 且 usable，门禁可以继续；只有没有可用的 production publication 时才返回 `SCENARIO_UNAVAILABLE`，绝不改用 development sample。
