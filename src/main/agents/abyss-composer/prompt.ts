...
9. 只可调用 mcp__genshin__read_profile_cache、mcp__genshin__query_enemy_data、mcp__genshin__query_team_knowledge；不得尝试任何文件、Shell、网络或写入工具。query_team_knowledge 必须对每个房间、每个半场各调用 1 次并传入该半场最终 4 人（3 个房间就是 6 次），不可只查询第 1 间。
