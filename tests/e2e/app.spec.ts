...
async function expectNoUnavailableGuidance(): Promise<void> {
  for (const text of [
...
test('runs the abyss-specific development-sample flow with accessible interventions and fixed twin teams', async () => {
  ...
  await expect(
    page.getByText('这支队伍是已校验方案的上半选择；先完成辅助布置，再进入主要输出窗口。', {
      exact: true
    })
  ).toBeVisible();
  await expect(
    page.getByText('这部分对应已校验的 12 层第 1 间上半目标；按本地方案完成布置后进入输出。', {
      exact: true
    })
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      'This section is tied to the validated Floor 12, Chamber 1, first-half target; complete setup before committing to damage.',
      { exact: true }
    )
  ).toBeVisible();
  await expectNoUnavailableGuidance();
...
test('plans three Stygian phases from the development scenario without leaking raw service keys', async () => {
  ...
  await expect(
    page.getByText(
      'The saved Phase 1 team is the validated local allocation; finish setup before the main damage window.',
      { exact: true }
    )
  ).toBeVisible();
  await expectNoUnavailableGuidance();
...
test('checks Theater eligibility and renders a cast-vigor route instead of team cards', async () => {
  ...
  await expect(
    page.getByText('这份演员池来自已校验的分配；保留调度余量，供后续幕次按计划轮换。', {
      exact: true
    })
  ).toBeVisible();
  await expect(
    page.getByText('第 1 幕的演员安排来自已校验的活力账本；本幕消耗不超过保存的预算。', {
      exact: true
    })
  ).toBeVisible();
  await expectNoUnavailableGuidance();
  ...
  await expect(
    page.getByText(
      'The Act 1 assignment follows the validated Vigor ledger and stays within its saved budget.',
      { exact: true }
    )
  ).toBeVisible();
  await expectNoUnavailableGuidance();
